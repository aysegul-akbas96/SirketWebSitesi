...
class Hizmet extends CI_Controller {
	...
	public function index()
	{
      ...
      $query=$this->db->query("SELECT * FROM siteayarlari");
		...
		$query=$this->db->query("SELECT * FROM resim inner join menu on resim.menu_id=menu.menu_id where menu.menu_id=2");
		$data["resim"]=$query->result();

		$query=$this->db->query("SELECT * FROM siteayarlari");
		$data["v"]=$query->result();
		$query=$this->db->query("SELECT menu_adi from menu where parent_id=1");
