...
class Ymenu extends CI_Controller {
	...
	public function index()
	{
      ...
      $query=$this->db->query("SELECT * FROM siteayarlari");
		...
		$query=$this->db->query("SELECT * FROM menu_alt inner join menu on menu.parent_id=menu_alt.parent_id where menu_alt.parent_id=5 order by menu_alt_id asc ");
		$data["menuismi"]=$query->result();

		$query=$this->db->query("SELECT * FROM resim inner join menu on resim.menu_id=menu.menu_id where menu.menu_id=6");
		$data["resim"]=$query->result();

		$query=$this->db->query("SELECT * FROM siteayarlari");
		$data["v"]=$query->result();
		$query=$this->db->query("SELECT menu_adi from menu where parent_id=5");
