...
class Sektor extends CI_Controller {
	...
	{
		$query=$this->db->query("SELECT * FROM menu_alt where parent_id=0");
		...
		$this->load->view('admin/sektor_listele',$data);
	...
	{      $query=$this->db->query("SELECT * FROM menu where menu_id=1");
		...
		$this->load->view('admin/sektor_ekle',$data);
...
public function ekle_kaydet()
	{
		...
		if ( !$this->upload->do_upload('menu_alt_resim'))//bu resim formdaki name="resim" den geliyor
        ...
        else
            {
			   ...
			   redirect(base_url()."admin/sektor");
 ...
 $query=$this->db->query("SELECT * FROM menu where menu_id=1");
		...
		$this->load->view('admin/sektor_duzenle',$data);
