...
class Medya extends CI_Controller {
...
where menu_alt.parent_id =2 group by menu_alt.menu_alt_id ");
		$data["menuismi"]=$query->result();


		$query=$this->db->query("SELECT resim.resim
FROM menu_alt
INNER JOIN menu ON menu.parent_id = menu_alt.parent_id
INNER JOIN resim ON resim.menu_alt_id = menu_alt.menu_alt_id
WHERE menu_alt.parent_id =2");
		$data["ab"]=$query->result();


		$query=$this->db->query("SELECT * FROM siteayarlari");
		$data["v"]=$query->result();
		$query=$this->db->query("SELECT menu_adi from menu where parent_id=2");
...
public function altgetir($id)
	{
		...
		$query=$this->db->query("SELECT * FROM menu_alt inner join menu on menu.parent_id=menu_alt.parent_id where menu_alt.parent_id=1 order by menu_alt_id asc ");
		$data["menuismi"]=$query->result();
